...
namespace eudaq {
  ...
  void FileWriterTree::WriteEvent(const DetectorEvent & ev) {
    ...
    for (unsigned int iEvent = 0; iEvent < ev.NumEvents(); ++iEvent) {
      ...
      if( data.size()-4!=unpack_fh(it,EVENT_LENGTH) || (data.size()!=140703 && data.size()!=140697 && data.size()!=140698)){
        ...
        std::cout<<"Event Length not correct!"<<std::endl<<"Either order not correct or Event corrupt!!"
                 <<std::endl<<"Expected Length: "<<EVENT_LENGTH<<" Seen: "<<data.size()-4<<" Correct Value: 140698"<<std::endl;
        return;
      }
      ...
      while(it!=data.end()){
        if(unpack_fh(it,FRAME_LENGTH)==9 || FRAME_LENGTH==4){                      //EOE expection
          if(FRAME_LENGTH==9){
            ...
            cout<<"EOE not correct!"<<endl<<"Found: "<<evn<<" instead of "<<EVENT_NUM<<endl
                <<"Dropping Event"<<endl;
            bad=true;
            break;
          }
          ...
          if(unpack_fn(it,evn)!=EVENT_NUM){               //EvNum exception
            cout<<"Eventnumber missmatch!\n"<<"Found "<<evn<<endl<<"Dropping Event!"<<endl;
            bad=true;
            break;
          }

          if( unpack_b(it,ds,3)!=DS ){
            cout<<"Data specifier do not fit!"<<endl<<"Continuing anyway!!"<<endl;
          }

          //if everything went correct up to here the next byte should be the Framecount
          if((int)*it<=0 || (int)*it>16){
            cout<<"Framecount not possible! <1.|==1.|>16"<<endl<<"Dropping Event"<<endl;
            bad=true;
            break;
          }
          ...
          if(nframes>17){
            cout<<"To many frames!"<<endl<<"Dropping Event"<<endl;
            bad=true;
            break;
          }
      ...
      nframes=0;              //reset
      vector<unsigned char>::iterator itend;    //iterator to indicate the end of datafr
