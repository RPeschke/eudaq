...
namespace eudaq {
  ...
  {
    std::cout << "EUDAQ_DEBUG: This is FileWriterTextdetector::FileWriterTextCompact("<< param <<")"<< std::endl;
  }

  void FileWriterTextdetector::StartRun(unsigned runnumber) {
    std::cout << "EUDAQ_DEBUG: FileWriterTextdetector::StartRun("<< runnumber << ")" << std::endl;
    // close an open file
	if (m_out)
	{
		m_out->close();
