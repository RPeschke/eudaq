...
class xmlProducer : public eudaq::Producer {
  public:

    // The constructor must call the eudaq::Producer constructor with the name
    // and the runcontrol connection string, and initialize any member variables.
    xmlProducer(const std::string & name, const std::string & runcontrol, const std::string &inFile)
...
int main(int /*argc*/, const char ** argv) {
  ...
  eudaq::OptionParser op("EUDAQ xml Producer", "1.0",
      "reads in a pseudo XML file and sends the data to the runcontrol like any other producer would do");
  eudaq::Option<std::string> rctrl(op, "r", "runcontrol",
      "tcp://localhost:44000", "address",
      "The address of the RunControl.");
  eudaq::Option<std::string> level(op, "l", "log-level", "NONE", "level",
      "The minimum level for displaying log messages locally");
  eudaq::Option<std::string> inFile(op, "f", "input-file", "NONE", "String",
	  "set the input file");
  ...
  try {
   ...
   xmlProducer producer(name.Value(), rctrl.Value(),inFile.Value());
    // And set it running...
