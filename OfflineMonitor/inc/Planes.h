...
public:
	...
	void setHistogramSize(double XLowerBin, double XUpperBin,int XnumberOfBins,double YLowerBin, double YUpperBin,int YnumberOfBins);
	~plane();
	bool nextEntry() ;
    void firstEntry() ;
	void addCurrentEntry(const int& plane_id,const int& hit_x,const int& hit_y);
	void addCurrentEntry2CalibrationData(const int& plane_id,const int& hit_x,const int& hit_y);
...
bool ignorRegin(double h,const std::vector<double> & beginVec,const std::vector<double> & endVec);
#endif // Planes_h__
